...
from app.lambdas import ingestion_lambda
...
def test_ingestion_lambda_writes_recon_record(monkeypatch):
    # Arrange env
    ...
    fake_repo = FakeS3Repo()

    # Patch S3ObjectStorageRepository to return our fake repo
    def fake_s3_repo_ctor() -> FakeS3Repo:
        return fake_repo

    monkeypatch.setattr(ingestion_lambda, "S3ObjectStorageRepository", fake_s3_repo_ctor)
    monkeypatch.setattr(ingestion_lambda, "configure_logging", lambda *args, **kwargs: None)
    ...
    # Act
    result = ingestion_lambda.handler(event, context=None)

    # Assert
    assert result["results"][0]["outcome"] == "REGISTERED"
    assert fake_repo.written, "Ingestion lambda should write a recon record"
    ...
    assert body["job_type"] == "INGESTION"
    assert body["table_name"] == "utilization_history"
    assert body["logical_period"] == "2025-01"
